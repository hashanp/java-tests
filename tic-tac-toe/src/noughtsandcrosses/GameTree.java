...
public class GameTree implements GameTreeInterface {
	...
	private int sizeTree(GameTreeNode node){
		int sum = 0;
		for (GameTreeNode subnode: node.getChildren()) {
		  sum += 1 + sizeTree(subnode);
    }
    return sum;
	}
	...
	private void expandTree(GameTreeNode node){
	  ...
	  if (!board.isFinished()) {
	    char c = board.getTurn();
	    for (int i = 1; i <= 9; i++) {
        if (board.getMark(i) == ' ') {
          final Board copy = board.makeCopy();
          copy.setMark(i, c);
          final GenericList<GameTreeNode> list = node.getChildren();
          final GameTreeNode node1 = new GameTreeNode(copy);
          list.add(list.size() + 1, node1);
          expandTree(node1);
        }
      }
    }
